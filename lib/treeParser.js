...
TreeParser.prototype.getId = function(name){
	...
	return name.replace(/^[0-9]+_/g, "").replace(/^_/, "").replace(/_/g, "-").trim().toLowerCase();
	...
	var parseDir = function(dirname, parent){
		...
		for(var i in files){
			...
			if(stats.isDirectory()){
				...
				//Title
				if(config.page_title) parent.title = config.page_title;
...
/**
 * Returns pages which differs from cache and modified files
 *
 * @param Object cache (optional)
 * @return Array
 */
TreeParser.prototype.getChanges = function(cache){
